methods {
    ...
    function balanceOf(address token, SablierFlow owner) external returns (uint256) envfree optional;
    function allowance(address token, address owner, address spender) external returns (uint256) envfree optional;
    function allowance(address token, address owner, SablierFlow spender) external returns (uint256) envfree optional;
}
...
// In the real protocol, stream IDs start at 1 and are allocated monotonically from `nextStreamId`.
invariant streamIdPositive(uint256 streamId)
    isStream(streamId) => streamId > 0;

invariant streamIdLessThanNext(uint256 streamId)
    isStream(streamId) => streamId < nextStreamId();

invariant tokenNonZeroForStreams(uint256 streamId)
    isStream(streamId) => getToken(streamId) != 0x0;
...
rule aggregateBalanceConservation(env e, uint256 streamId, uint128 amount, address sender, address recipient) {
    ...
    require allowance(token, sender, currentContract) >= amount, "sender approved Flow contract";
...
rule withdrawalAlwaysSucceeds(env e, uint256 streamId, address to, uint128 amount) {
    ...
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";
    require balanceOf(token, currentContract) >= amount, "Flow contract has enough tokens to withdraw";
...
rule refundAlwaysSucceeds(env e, uint256 streamId, uint128 amount) {
    ...
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";
    require balanceOf(token, currentContract) >= amount, "Flow contract has enough tokens to refund";
