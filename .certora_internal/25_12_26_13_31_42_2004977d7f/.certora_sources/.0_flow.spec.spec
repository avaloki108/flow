methods {
    ...
    // ========= ERC-721 surface (avoid env/arity confusion with ERC20 balanceOf) =========
    function balanceOf(address owner) external returns (uint256) envfree;

    // ========= External ERC20 model (ERC20Mock) =========
    // Model calls against the token contract; the first argument is the token address.
    // Marked `optional` so the prover models unresolved token behavior without requiring
    // a matching method on SablierFlow itself.
    function balanceOf(address token, address owner) external returns (uint256) envfree optional;
    function allowance(address token, address owner, address spender) external returns (uint256) envfree optional;
}
...
rule voidIsPermanent(env e, uint256 streamId, method f) {
    require isStream(streamId), "existing stream";
    require streamId > 0, "streamId 0 is unreachable in real deployments";
    require streamId < nextStreamId(), "stream ids are allocated from nextStreamId and never reused";
    // Exclude `batch(bytes[])` (selector 0x1e897afb): it uses `delegatecall` on dynamic calldata and the Prover
    // may apply an AUTO-havoc summary, spuriously "un-voiding" streams.
    require f.selector != 0x1e897afb;
...
// ============ WELL-FORMED STREAM ASSUMPTIONS ============
//
// Express these as rules (rather than invariants) so we can exclude `batch(bytes[])`, which otherwise
// can introduce an AUTO-havoc summary that makes the induction checks meaningless.

rule streamIdPositiveAlways(env e, uint256 streamId, method f) {
    require f.selector != 0x1e897afb;
    calldataarg args;
    f@withrevert(e, args);
    assert isStream(streamId) => streamId > 0;
}

rule streamIdLessThanNextAlways(env e, uint256 streamId, method f) {
    require f.selector != 0x1e897afb;
    calldataarg args;
    f@withrevert(e, args);
    assert isStream(streamId) => streamId < nextStreamId();
}

rule tokenNonZeroForStreamsAlways(env e, uint256 streamId, method f) {
    require f.selector != 0x1e897afb;
    calldataarg args;
    f@withrevert(e, args);
    assert isStream(streamId) => getToken(streamId) != 0x0;
}
...
rule streamIdMonotonicity(env e, method f) {
    ...
    require idBefore < 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff,
        "exclude theoretical uint256 overflow (unreachable in practice)";
    // Exclude `batch(bytes[])` (selector 0x1e897afb): it uses `delegatecall` on dynamic calldata and the Prover
    // may apply an AUTO-havoc summary that arbitrarily mutates `nextStreamId`.
    require f.selector != 0x1e897afb;
    calldataarg args;
    f@withrevert(e, args);
    bool reverted = lastReverted;
    uint256 idAfter = nextStreamId();
    assert reverted => idAfter == idBefore, "reverts must not mutate nextStreamId";
    assert !reverted => idAfter >= idBefore, "nextStreamId should never decrease";
}
...
rule aggregateBalanceConservation(env e, uint256 streamId, uint128 amount, address sender, address recipient) {
    ...
    require streamId < nextStreamId(), "stream ids are allocated from nextStreamId and never reused";
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";

    // Constrain to a "well-formed ERC20 transferFrom" path so deposit can actually succeed
    // under the ERC20Mock model (otherwise the prover can pick unreachable states where the
    // Flow contract's accounting says funds exist, but the token's balance/allowance does not).
    require balanceOf(token, sender) >= amount, "sender has enough token balance";
    address self = currentContract;
    require allowance(token, sender, self) >= amount, "sender approved Flow contract";
...
rule withdrawalAlwaysSucceeds(env e, uint256 streamId, address to, uint128 amount) {
    ...
    require streamId > 0, "streamId 0 is unreachable in real deployments";
    require streamId < nextStreamId(), "stream ids are allocated from nextStreamId and never reused";
    require to == getRecipient(streamId),
        "withdraw can revert if `to` is not the recipient and caller is not recipient/approved";
    require e.msg.value >= calculateMinFeeWei(e, streamId),
        "withdraw requires paying at least the comptroller min fee for the stream sender";
    uint128 withdrawable = withdrawableAmountOf(e, streamId);
    require withdrawable >= amount, "only test the 'valid amount' path";
    require amount > 0, "withdraw(0) is specified to revert";

    // Same rationale as above: restrict to reachable states under ERC20Mock.
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";
    address self = currentContract;
    require balanceOf(token, self) >= amount, "Flow contract has enough tokens to withdraw";
...
rule refundAlwaysSucceeds(env e, uint256 streamId, uint128 amount) {
    ...
    require streamId > 0, "streamId 0 is unreachable in real deployments";
    require streamId < nextStreamId(), "stream ids are allocated from nextStreamId and never reused";
    require e.msg.sender == getSender(streamId), "refund is restricted to the stream sender";
    uint128 refundable = refundableAmountOf(e, streamId);
    require refundable >= amount, "only test the 'valid amount' path";
    require amount > 0, "refund(0) is specified to revert";

    // Restrict to reachable states under ERC20Mock.
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";
    address self = currentContract;
    require balanceOf(token, self) >= amount, "Flow contract has enough tokens to refund";
