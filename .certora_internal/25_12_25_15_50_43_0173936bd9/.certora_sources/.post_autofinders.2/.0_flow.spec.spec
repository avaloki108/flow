methods {
    ...
    function ERC20Mock.balanceOf(address owner) external returns (uint256) envfree;
    function ERC20Mock.allowance(address owner, address spender) external returns (uint256) envfree;
}
...
rule aggregateBalanceConservation(env e, uint256 streamId, uint128 amount, address sender, address recipient) {
    ...
    require ERC20Mock.balanceOf(token, sender) >= amount, "sender has enough token balance";
    require ERC20Mock.allowance(token, sender, currentContract) >= amount, "sender approved Flow contract";
...
rule withdrawalAlwaysSucceeds(env e, uint256 streamId, address to, uint128 amount) {
    ...
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";
    require ERC20Mock.balanceOf(token, currentContract) >= amount, "Flow contract has enough tokens to withdraw";
...
rule refundAlwaysSucceeds(env e, uint256 streamId, uint128 amount) {
    ...
    address token = getToken(streamId);
    require token != 0x0, "stream token is never the zero address";
    require ERC20Mock.balanceOf(token, currentContract) >= amount, "Flow contract has enough tokens to refund";
