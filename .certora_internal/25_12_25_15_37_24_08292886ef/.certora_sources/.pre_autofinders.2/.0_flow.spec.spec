methods {
    ...
    // These are not methods on SablierFlow, so they must be declared `optional`.
    // They are invoked as balanceOf(token, owner) / allowance(token, owner, spender).
    function balanceOf(address owner) external returns (uint256) envfree optional;
    function allowance(address owner, address spender) external returns (uint256) envfree optional;
}
