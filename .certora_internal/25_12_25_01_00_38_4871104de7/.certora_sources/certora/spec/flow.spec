...
// Withdraw reduces withdrawable balance by at least the amount withdrawn
rule withdrawDecreasesWithdrawable(env e, uint256 streamId, uint128 amount, address to) {
    uint128 before = withdrawableAmountOf(streamId);
    require amount > 0;
    withdraw@withrevert(e, streamId, to, amount);
    assert !lastReverted => withdrawableAmountOf(streamId) + amount <= before, "withdrawable must decrease by amount";
}
...
// Refund reduces refundable amount by at least the refunded amount
rule refundDecreasesRefundable(env e, uint256 streamId, uint128 amount) {
    uint128 before = refundableAmountOf(streamId);
    require amount > 0;
    refund@withrevert(e, streamId, amount);
    assert !lastReverted => refundableAmountOf(streamId) + amount <= before, "refundable must decrease by amount";
}
